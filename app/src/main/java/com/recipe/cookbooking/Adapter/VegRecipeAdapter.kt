...
class VegRecipeAdapter(private var recipeList: List<Recipe>, private val context: Context) :
    RecyclerView.Adapter<VegRecipeAdapter.VegRecipeViewHolder>() {

    class VegRecipeViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val tvVegLabel: TextView = itemView.findViewById(R.id.tvVegLabel)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VegRecipeViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_veg_recipe, parent, false)
        return VegRecipeViewHolder(view)
    }

    override fun onBindViewHolder(holder: VegRecipeViewHolder, position: Int) {
        val recipe = recipeList[position]
        holder.tvRecipeName.text = recipe.name
        holder.tvVegLabel.text = recipe.subCategory
